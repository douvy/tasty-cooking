...
document.querySelectorAll('[data-tag]').forEach(function(element) {
    element.addEventListener('click', function(event) {
        ...
        if (event.target.classList.contains('bg-[#222616]')) {
            event.target.classList.remove('bg-[#222616]', 'text-off-white');
            removeTagLabel(tag);
        } else {
            event.target.classList.add('bg-[#222616]', 'text-off-white');
            addTagLabel(tag);
        }
...
function addTagLabel(tag) {
    ...
    tagLabel.className = 'inline-flex items-center px-2.5 py-1 sm:py-2 text-sm rounded-sm bg-[#2e3523] capitalize text-off-white mt-2 mr-2 sm:mr-2 cursor-pointer hover:bg-light-gray';
...
function removeActiveState(tag) {
    document.querySelectorAll('[data-tag]').forEach(element => {
        if (element.getAttribute('data-tag') === tag) {
            element.classList.remove('bg-[#141519]', 'text-off-white');
        }
    });
}
...
if (visibleCount === 0) {
    if (!noResultsMsg) {
        ...
        noResultsMsg.className = 'col-span-full text-center py-16 text-off-white text-xl sm:text-2xl';
