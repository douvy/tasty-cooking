...
document.querySelectorAll('[data-tag]').forEach(function(element) {
    // First, update all elements to have either active or hover state
    if (element.classList.contains('bg-[#141519]')) {
        // For active elements, set hover to match active
        element.classList.remove('hover:bg-[#1b1c21]');
        element.classList.add('hover:bg-[#141519]');
    }
    ...
    element.addEventListener('click', function(event) {
        ...
        if (event.target.classList.contains('bg-[#141519]')) {
            ...
            event.target.classList.remove('bg-[#141519]');
            event.target.classList.remove('hover:bg-[#141519]');
            event.target.classList.add('hover:bg-[#25272c]');
            removeTagLabel(tag);
        } else {
            // If not active, activate
            event.target.classList.add('bg-[#141519]');
            event.target.classList.remove('hover:bg-[#25272c]');
            event.target.classList.add('hover:bg-[#141519]');
            addTagLabel(tag);
        }
...
function addTagLabel(tag) {
    ...
    tagLabel.className = 'inline-flex items-center px-3.5 py-1.5 rounded-sm text-sm font-medium bg-[#33353c] capitalize text-white mr-2 mb-1 mt-2 cursor-pointer hover:bg-[#3b3d44]';
    tagLabel.innerHTML = `${tag} <button type="button" class="ml-2 text-white" aria-label="Remove tag"><i class="fas fa-times font-light"></i></button>`;
    ...
    // No need to change icon color on hover since it's already white
    // But we'll keep the hover events for potential future styling
    tagLabel.addEventListener('mouseover', function() {
        // Already styled through hover:bg-[#292b31]
    });
...
function removeActiveState(tag) {
    document.querySelectorAll('[data-tag]').forEach(element => {
        if (element.getAttribute('data-tag') === tag) {
            element.classList.remove('bg-[#141519]');
            element.classList.remove('hover:bg-[#141519]');
            element.classList.add('hover:bg-[#25272c]');
        }
    });
}
...
function filterRecipes() {
    ...
    if (visibleCount === 0) {
        if (!noResultsMsg) {
            ...
            noResultsMsg.style.top = '25%';
            noResultsMsg.style.left = '50%';
            noResultsMsg.style.transform = 'translate(-50%, -50%)';
            noResultsMsg.style.width = '100%';
            noResultsMsg.style.textAlign = 'center';
            noResultsMsg.style.zIndex = '10';
            noResultsMsg.style.pointerEvents = 'none'; // Allow clicking through to the tags
            noResultsMsg.innerHTML = '<p class="text-white text-2xl font-windsor-bold whitespace-nowrap">No recipes match the selected filters</p><p class="mt-2 text-base text-gray">Try removing some tags to see more results</p>';
...
document.querySelectorAll('[data-sort]').forEach(function(element) {
    ...
    element.addEventListener('click', function(event) {
        ...
        // Remove active class from all sort options and reset hover effects
        document.querySelectorAll('[data-sort]').forEach(el => {
            el.classList.remove('bg-[#141519]');
            el.classList.remove('hover:bg-[#141519]');
            el.classList.add('hover:bg-[#25272c]');
        });

        // Add active class to the selected sort option and set hover to match active
        event.target.classList.add('bg-[#141519]');
        event.target.classList.remove('hover:bg-[#25272c]');
        event.target.classList.add('hover:bg-[#141519]');
...
searchBar.addEventListener('input', function() {
    ...
    if (query) {
        ...
        // Ensure search results width matches search bar width on desktop
