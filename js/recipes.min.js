...
function addTagLabel(tag) {
    ...
    tagLabel.className = 'inline-flex items-center px-2.5 py-1 sm:py-2 text-sm rounded-sm bg-[#3b3d44] capitalize text-white mt-2 mr-2 sm:mr-2 cursor-pointer hover:bg-light-gray';
...
if (visibleCount === 0) {
    if (!noResultsMsg) {
        ...
        noResultsMsg.className = 'col-span-full text-center py-16 text-white text-xl sm:text-2xl';
        noResultsMsg.innerHTML = '<p class="font-windsor-bold">No recipes match the selected filters</p><p class="mt-1 text-base sm:text-lg text-gray">Try removing some tags to see more results</p>';
...
function updateHighlightedResult(results) {
    results.forEach((result, index) => {
        if (index === currentIndex) {
            // Apply the color directly as inline style for consistency
