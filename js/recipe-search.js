        ...
        if (query.length > 0) {
            ...
            if (filteredRecipes.length > 0) {
                ...
                filteredRecipes.forEach(recipe => {
                    ...
                    div.className = 'px-4 py-2 cursor-pointer hover:bg-gray flex items-center space-x-2 border-b border-dark-gray-all w-full';
                    div.innerHTML = `
                        <img src="${recipe.img}" alt="${recipe.title}" class="w-5 h-5 rounded-sm object-cover"> 
                        <span class="text-gray capitalize truncate">${recipe.title.toLowerCase()}</span>
                    `;
